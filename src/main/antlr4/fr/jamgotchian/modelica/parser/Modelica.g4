...
// translated to ANTLR from appendix B of https://www.modelica.org/documents/ModelicaSpec33.pdf

// B.1 Lexical convention
...
fragment UNSIGNED_INTEGER : DIGIT (DIGIT)*
                 ;
...
composition : element_list ('public' element_list | 'protected' element_list | equation_section | algorithm_section)*
                           ('external' (language_specification)? (external_function_call)? (annotation)? ';')? (annotation ';')?
            ;
...
import_clause : 'import' (IDENT '=' name | name ('.' ('*' | '{' import_list '}'))?) comment
              ;

import_list : IDENT (',' import_list)?
            ;
...
element_redeclaration : 'redeclare' ('each')? ('final')? ((short_class_definition | component_clause1) | element_replaceable)
                      ;
...
short_class_definition : class_prefixes IDENT '=' (base_prefix name (array_subscripts)? (class_modification)? comment | 'enumeration' '(' ((enum_list)? | ':' ) ')' comment)
                       ;
